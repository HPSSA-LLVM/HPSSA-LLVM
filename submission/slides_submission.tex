...
\end{frame}
{
	...
	\includepdf[pages={3,4,5}]{summary.pdf}
}
\begin{frame}
	...
	\begin{itemize}
		...
		\item Modified the existing SCCP Pass to add in a function which handles the special \mintinline[fontsize=\footnotesize]{css}{"llvm.tau"} intrinsic instructions used for $\tau$-functions. \color{blue} (8 lines extra of code only!) \color{black}
		\item Added a new lattice element type \mintinline[fontsize=\footnotesize]{css}{"spec_constant"} in \mintinline[fontsize=\footnotesize]{css}{ValueLattice} class supporting operations on speculative constants. \color{blue} (5 lines extra of code only!) \color{black} 
		\item Added new functions in the \mintinline[fontsize=\footnotesize]{css}{SCCPInstVisitor} and \mintinline[fontsize=\footnotesize]{css}{SCCPSolver} class to handle operations on speculative constants. Eg. Operands can be marked speculative using \mintinline[fontsize=\footnotesize]{css}{markSpeculativeConstant()} function. \color{blue} (5 lines extra of code only!) \color{black}
	\end{itemize}
\end{frame}
