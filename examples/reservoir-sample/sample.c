...
int main(int argc, char **argv) {
  ...
  if (sample_without_replacement) {
    if ((hybrid_in_file) || (cstdio_in_file)) {
      ...
      if (sample_size_specified)
        sample_reservoir_offsets_without_replacement_via_cstdio_with_fixed_k(
            in_file_ptr, &offset_reservoir_ptr, lines_per_offset);
      else {
        sample_reservoir_offsets_without_replacement_via_cstdio_with_unspecified_k(
            in_file_ptr, &offset_reservoir_ptr, lines_per_offset);
      ...
      if (sample_size_specified)
        sample_reservoir_offsets_without_replacement_via_mmap_with_fixed_k(
            in_file_mmap_ptr, &offset_reservoir_ptr, lines_per_offset);
      else {
        sample_reservoir_offsets_without_replacement_via_mmap_with_unspecified_k(
            in_file_mmap_ptr, &offset_reservoir_ptr, lines_per_offset);
        shuffle_reservoir_offsets_via_fisher_yates(&offset_reservoir_ptr);
      }
    }
    ...
    if ((hybrid_in_file) || (cstdio_in_file)) {
      ...
      sample_reservoir_offsets_without_replacement_via_cstdio_with_fixed_k(
          in_file_ptr, &offset_reservoir_ptr, lines_per_offset);
      if (sample_size_specified)
        sample_reservoir_offsets_with_replacement_via_cstdio_with_fixed_k(
            &offset_reservoir_ptr, k);
      else
        sample_reservoir_offsets_with_replacement_via_cstdio_with_unspecified_k(
            &offset_reservoir_ptr);
    } else if (mmap_in_file) {
      in_file_mmap_ptr = new_file_mmap(in_filename);
      sample_reservoir_offsets_without_replacement_via_mmap_with_unspecified_k(
          in_file_mmap_ptr, &offset_reservoir_ptr, lines_per_offset);
      if (sample_size_specified)
        sample_reservoir_offsets_with_replacement_via_mmap_with_fixed_k(
            &offset_reservoir_ptr, k);
      else
        sample_reservoir_offsets_with_replacement_via_mmap_with_unspecified_k(
            &offset_reservoir_ptr);
    }
  }
  ...
  if (hybrid_in_file)
    print_offset_reservoir_sample_via_mmap(
        in_file_mmap_ptr, offset_reservoir_ptr, lines_per_offset);
...
void print_offset_reservoir_ptr(const offset_reservoir *res_ptr) {
  ...
  for (idx = 0; idx < res_ptr->num_offsets; ++idx)
    fprintf(stdout, "[%012d] %012lld\n", idx,
            (long long int)res_ptr->offsets[idx]);
...
void sample_reservoir_offsets_without_replacement_via_cstdio_with_fixed_k(
  ...
  /* read offsets into reservoir, replacing random offsets when the
   * line-grouping counter is greater than k */
...
#ifdef DEBUG
        fprintf(stderr,
                "Debug: Replacing random offset %012ld for line %012ld with "
                "probability %f\n",
...
void sample_reservoir_offsets_without_replacement_via_cstdio_with_unspecified_k(
  ...
  while (fgets(in_line, LINE_LENGTH_VALUE + 1, in_file_ptr)) {
    ...
    if (grp_idx == k) {
      ...
      if (!resized_offsets) {
        fprintf(stderr,
                "Error: Could not allocate memory for resized offset array\n");
        exit(EXIT_FAILURE);
      }
  ...
  for (offset_idx = 0; offset_idx < in_mmap->size; ++offset_idx) {
    if (in_mmap->map[offset_idx] == '\n') {
      ...
      if (grp_idx < k) {
        ...
        fprintf(
            stderr,
            "Debug: Adding offset at idx %012ld with offset value %012lld\n",
            grp_idx, (long long int)start_offset);
#endif
        (*res_ptr)->offsets[grp_idx] = start_offset;
      } else {
...
#ifdef DEBUG
  fprintf(stderr, "Debug: Entering --> "
                  "sample_reservoir_offsets_without_replacement_via_mmap_with_"
                  "unspecified_k()\n");
#endif
...
void sample_reservoir_offsets_with_replacement_with_fixed_k(
  ...
  */
  for (sample_offset_idx = 0; sample_offset_idx < sample_size - 1;
       ++sample_offset_idx) {
    original_random_idx =
        mt19937_generate_random_double() * original_sample_size;
    sample_offset_reservoir_ptr->offsets[sample_offset_idx] =
        original_offset_reservoir_ptr->offsets[original_random_idx];
  }
...
void shuffle_reservoir_offsets_via_fisher_yates(offset_reservoir **res_ptr) {
  ...
  /* cf. http://blog.codinghorror.com/the-danger-of-naivete/ for an interesting
   * discussion about Fisher-Yates */
...
#ifdef DEBUG
  fprintf(stderr,
          "Debug: Leaving  --> shuffle_reservoir_offsets_via_fisher_yates()\n");
#endif
}
...
void sort_offset_reservoir_ptr_offsets(offset_reservoir **res_ptr) {
  ...
  qsort((*res_ptr)->offsets, (*res_ptr)->num_offsets, sizeof(off_t),
        offset_compare);
...
void print_offset_reservoir_sample_via_mmap(const file_mmap *in_mmap,
...
#ifdef DEBUG
  fprintf(stderr,
          "Debug: Entering --> print_offset_reservoir_sample_via_mmap()\n");
#endif
  ...
  for (res_idx = 0; res_idx < res_ptr->num_offsets; ++res_idx) {
    ...
    for (mmap_idx = current_offset;
         mmap_idx < current_offset + LINE_LENGTH_VALUE; ++mmap_idx) {
...
#ifdef DEBUG
  fprintf(stderr,
          "Debug: Leaving  --> print_offset_reservoir_sample_via_mmap()\n");
#endif
}
...
void print_sorted_offset_reservoir_sample_via_cstdio(
  ...
  for (idx = 0; idx < res_ptr->num_offsets; ++idx) {
    ...
    */
    fseek(in_file_ptr,
          res_ptr->offsets[idx] - previous_offset - previous_line_length,
          SEEK_CUR);
...
void print_unsorted_offset_reservoir_sample_via_cstdio(
  ...
  for (idx = 0; idx < res_ptr->num_offsets; ++idx) {
    /*
       we use SEEK_SET to jump from the start of the file, as the offsets are
       unsorted
    */
...
file_mmap *new_file_mmap(const char *in_fn) {
  ...
  if (!not_stdin) {
    fprintf(stderr,
            "Error: Stdin not yet supported with mmap setup function\n");
    exit(EXIT_FAILURE);
  }
...
void parse_command_line_options(int argc, char **argv) {
  ...
  int client_opt = getopt_long(argc, argv, sample_client_opt_string,
                               sample_client_long_options, &client_long_index);
  ...
  while (client_opt != -1) {
    ...
    case 'k':
      if (optarg) {
        ...
        fprintf(stderr, "Error: Sample size option is specified, but its value "
                        "is unspecified\n");
        print_usage(stderr);
        exit(EXIT_FAILURE);
      }
    case 'l':
      if (optarg) {
        ...
        fprintf(stderr, "Error: Lines-per-offset option is specified, but its "
    ...
    case 'd':
      if (optarg) {
        ...
        fprintf(stderr, "Error: RNG seed initializer option is specified, but "
    ...
    client_opt = getopt_long(argc, argv, sample_client_opt_string,
                             sample_client_long_options, &client_long_index);
  }
  ...
  if ((order_type_flags > 1) || (sample_type_flags > 1) ||
      (io_type_flags > 1) || (sample_global_args.lines_per_offset < 1) ||
      (sample_global_args.k < 1) || (sample_global_args.num_filenames != 1) ||
      (sample_global_args.rng_seed_value < 1)) {
    print_usage(stderr);
    exit(EXIT_FAILURE);
  }
