...
auto randomLL = std::bind(random_attr, generator);
...
template <typename T> void PerformPrim(Graph<T> graph) {
  vector<T> Q;
  ...
  auto start = graph.vertices[randomLL()];
  key[start] = 0;
  while (!Q.empty()) {
    /* Choose the vertex with the minimum Key amongst all available vertices */
    auto u = *std::min_element(Q.begin(), Q.end(),
                               [&](char x, char y) { return key[x] < key[y]; });
    auto itr = remove(Q.begin(), Q.end(), u); // O(V)
    /* Remove the vertex and add it to minimum spanning tree. */
    Q.erase(itr, Q.end());
    if (parent[u] != '\0') {
      /* Since, we choose a vertex with the minimum key, the corresponding edge
       * should be in the minimum spanning tree. */
      min_spanning_tree[u] = parent[u];
    }
    /* Consider all the adjecent vertices of the graph. We update the key value
     * as we check-out each edge. */
...
template <typename T> class Disjoint_set {
  ...
  unordered_map<T, T> parent;
  unordered_map<T, int>
      rank; /* How many elements in set. How deep the tree is after union */
  std::vector<T> vset;
...
int main() {
  ...
  g.edges.emplace_back(Edge<char>('a', 'c', randomLL()));
