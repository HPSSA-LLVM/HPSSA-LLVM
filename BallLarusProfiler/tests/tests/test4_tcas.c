...
#define TCAS_IsInside()                                                        \
  ((ddh < Square(tcas_dmod) && ddv < Square(2 * tcas_alim)) ? 1 : 0)
...
void tcas_periodic_task_1Hz(void) {
  ...
  uint8_t i;
  float vx =
      stateGetHorizontalSpeedNorm_f() * sinf(stateGetHorizontalSpeedDir_f());
  float vy =
      stateGetHorizontalSpeedNorm_f() * cosf(stateGetHorizontalSpeedDir_f());
  for (i = 2; i < NB_ACS; i++) {
    if (ti_acs[i].ac_id == 0) {
      continue;
    } // no AC data
    ...
    if (dt > TCAS_DT_MAX) {
      continue;
    } // lost com but keep current status
    float dx =
        acInfoGetPositionEnu_f(ti_acs[i].ac_id)->x - stateGetPositionEnu_f()->x;
    float dy =
        acInfoGetPositionEnu_f(ti_acs[i].ac_id)->y - stateGetPositionEnu_f()->y;
    float dz =
        acInfoGetPositionEnu_f(ti_acs[i].ac_id)->z - stateGetPositionEnu_f()->z;
    float dvx = vx - acInfoGetVelocityEnu_f(ti_acs[i].ac_id)->x;
    float dvy = vy - acInfoGetVelocityEnu_f(ti_acs[i].ac_id)->y;
    float dvz =
        stateGetSpeedEnu_f()->z - acInfoGetVelocityEnu_f(ti_acs[i].ac_id)->z;
    ...
    case TCAS_RA:
      if (tau >= TCAS_HUGE_TAU && !inside) {
        ...
        tcas_acs_status[i].resolve = RA_NONE;
        DOWNLINK_SEND_TCAS_RESOLVED(DefaultChannel, DefaultDevice,
                                    &(ti_acs[i].ac_id));
      }
      break;
    case TCAS_TA:
      if (tau < tcas_tau_ra || inside) {
        ...
        // DOWNLINK_SEND_TCAS_RA(DefaultChannel,
        // DefaultDevice,&(ti_acs[i].ac_id),&test_dir);// FIXME only one closest
        // AC ???
        break;
      }
    ...
    case TCAS_NO_ALARM:
      if (tau < tcas_tau_ta || inside) {
        ...
        // Downlink warning
        DOWNLINK_SEND_TCAS_TA(DefaultChannel, DefaultDevice,
                              &(ti_acs[i].ac_id));
      }
      if (tau < tcas_tau_ra || inside) {
        ...
        // test_dir = tcas_test_direction(ti_acs[i].ac_id);
        // DOWNLINK_SEND_TCAS_RA(DefaultChannel,
        // DefaultDevice,&(ti_acs[i].ac_id),&test_dir);
      }
      break;
    default:
      break;
  ...
  if (tcas_status == TCAS_RA && tcas_ac_RA != AC_ID &&
      tcas_acs_status[ti_acs_id[tcas_ac_RA]].status == TCAS_RA) {
  ...
  if (tcas_status == TCAS_RA) {
    ...
    if (ac_resolve != RA_NONE) {        // first resolution, no message received
      ...
      tcas_acs_status[ti_acs_id[tcas_ac_RA]].resolve =
          RA_LEVEL;                 // assuming level flight for now
      ...
      if (ac_resolve != RA_LEVEL) { // message received
        ...
        if (tcas_resolve == RA_CLIMB &&
            ti_acs[ti_acs_id[tcas_ac_RA]].climb > 1.0) {
          tcas_resolve = RA_DESCEND;
        } // revert resolve
...
#ifdef TCAS_DEBUG
  if (tcas_status == TCAS_RA) {
    DOWNLINK_SEND_TCAS_DEBUG(DefaultChannel, DefaultDevice, &ac_id_close,
                             &tau_min);
  }
#endif
}
