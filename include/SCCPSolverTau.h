//===- SCCPTauSolver.h - SCCP Utility ----------------------------- *- C++ -*-===//
//
...
class SCCPTauInstVisitor;

//===----------------------------------------------------------------------===//
//
/// SCCPTauSolver - This interface class is a general purpose solver for Sparse
/// Conditional Constant Propagation (SCCP).
///
class SCCPTauSolver {
  std::unique_ptr<SCCPTauInstVisitor> Visitor;

public:
  SCCPTauSolver(const DataLayout &DL,
  ...
  /// inform the SCCPTauSolver that it should track loads and stores to the
