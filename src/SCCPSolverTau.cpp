//===- SCCPTauSolver.cpp - SCCP Utility --------------------------- *- C++ -*-===//
//
...
/// holds all the state.
class SCCPTauInstVisitor : public InstVisitor<SCCPTauInstVisitor> {
  const DataLayout &DL;
...
private:
  ...
  // markSpeculative - Make a value be marked as "speculative". If the
  // value is not already overdefined, add it to the overdefined instruction
  // work list so that the users of the instruction are updated later.
...
private:
  friend class InstVisitor<SCCPTauInstVisitor>;

  // visit implementations - Something changed in this instruction.  Either an
  // operand made a transition, or the instruction is newly executable.  Change
  // the value type of I to reflect these changes if appropriate.
  void visitPHINode(PHINode &I);
  void visitTauNode(Instruction &I);
...
public:
  ...
  SCCPTauInstVisitor(const DataLayout &DL,
                  std::function<const TargetLibraryInfo &(Function &)> GetTLI,
                  LLVMContext &Ctx)
  ...
  void markSpeculativeConstant(Value *V) {
    LLVM_DEBUG(dbgs() << "[Taulog] Marking Speculative Constant : " 
      << V->getValueName() << "\n");
    if (auto *STy = dyn_cast<StructType>(V->getType()))
      for (unsigned i = 0, e = STy->getNumElements(); i != e; ++i)
...
bool SCCPTauInstVisitor::markBlockExecutable(BasicBlock *BB) {
...
void SCCPTauInstVisitor::markArgInFuncSpecialization(Function *F, Argument *A,
                                                  Constant *C) {
...
void SCCPTauInstVisitor::visitInstruction(Instruction &I) {
  ...
  LLVM_DEBUG(dbgs() << "Spec SCCP: Don't know how to handle: " << I << '\n');
  markOverdefined(&I);
}
...
bool SCCPTauInstVisitor::markEdgeExecutable(BasicBlock *Source, BasicBlock *Dest) {
...
// getFeasibleSuccessors - Return a vector of booleans to indicate which
// successors are reachable from a given terminator instruction.
void SCCPTauInstVisitor::getFeasibleSuccessors(Instruction &TI,
...
bool SCCPTauInstVisitor::isEdgeFeasible(BasicBlock *From, BasicBlock *To) const {
...
void SCCPTauInstVisitor::visitTauNode(Instruction &Tau) {
  ...
  SpecValueLatticeElement x0 = getValueState(Tau.getOperand(0)), 
    beta = getValueState(Tau.getOperand(1)); // x0
  
  LLVM_DEBUG(dbgs() << "\t\tTau Operand (1) : " 
    << Tau.getOperand(1)->getNameOrAsOperand() << ", " << beta <<"\n");

  for (unsigned i = 2, e = Tau.getNumOperands(); i != e; ++i) {
  ...
  // limit multiple extensions caused by the same incoming value, if other
  // incoming values are equal.
  // Beta = x1 meet x2 meet x3 ....
  if (beta.isConstant() || beta.isConstantRange()) {
    // COMMENT : Beta is a safe constant. 
    LLVM_DEBUG(dbgs() << "\t\t [Taulog] Meet over Tau operands are constants.\n" 
      << TauState << "\n");
    beta.markSpeculativeConstant();
    TauState.mergeIn(beta);
  } 

  // COMMENT : Beta is not a safe constant here
  // phi "safe" operand is x0.
  x0.mergeIn(beta); // x0 <- x0 meet Beta
  if (!x0.isOverdefined()) // not bot
    TauState.mergeIn(x0);
  else 
    TauState.mergeIn(beta);
  ...
  LLVM_DEBUG(dbgs() << "\t\t [Taulog] ValueLattice (TauState) : " << TauState << "\n");
}
...
void SCCPTauInstVisitor::visitTerminator(Instruction &TI) {
...
void SCCPTauInstVisitor::visitCastInst(CastInst &I) {
...
void SCCPTauInstVisitor::visitExtractValueInst(ExtractValueInst &EVI) {
...
void SCCPTauInstVisitor::visitInsertValueInst(InsertValueInst &IVI) {
...
void SCCPTauInstVisitor::visitSelectInst(SelectInst &I) {
...
void SCCPTauInstVisitor::visitCmpInst(CmpInst &I) {
...
void SCCPTauInstVisitor::visitGetElementPtrInst(GetElementPtrInst &I) {
  if (isOverdefined(ValueState[&I]))
    return (void)markOverdefined(&I);
...
void SCCPTauInstVisitor::visitStoreInst(StoreInst &SI) {
...
void SCCPTauInstVisitor::visitLoadInst(LoadInst &I) {
...
/// them as overdefined.
bool SCCPTauInstVisitor::resolvedUndefsIn(Function &F) {
...
//
SCCPTauSolver::SCCPTauSolver(
    const DataLayout &DL,
    std::function<const TargetLibraryInfo &(Function &)> GetTLI,
    LLVMContext &Ctx)
    : Visitor(new SCCPTauInstVisitor(DL, std::move(GetTLI), Ctx)) {}

SCCPTauSolver::~SCCPTauSolver() {}

void SCCPTauSolver::addAnalysis(Function &F, AnalysisResultsForFn A) {
  return Visitor->addAnalysis(F, std::move(A));
}

bool SCCPTauSolver::markBlockExecutable(BasicBlock *BB) {
  return Visitor->markBlockExecutable(BB);
}

const PredicateBase *SCCPTauSolver::getPredicateInfoFor(Instruction *I) {
  return Visitor->getPredicateInfoFor(I);
}

DomTreeUpdater SCCPTauSolver::getDTU(Function &F) { return Visitor->getDTU(F); }

void SCCPTauSolver::trackValueOfGlobalVariable(GlobalVariable *GV) {
  Visitor->trackValueOfGlobalVariable(GV);
}

void SCCPTauSolver::addTrackedFunction(Function *F) {
  Visitor->addTrackedFunction(F);
}

void SCCPTauSolver::addToMustPreserveReturnsInFunctions(Function *F) {
  Visitor->addToMustPreserveReturnsInFunctions(F);
}

bool SCCPTauSolver::mustPreserveReturn(Function *F) {
  return Visitor->mustPreserveReturn(F);
}

void SCCPTauSolver::addArgumentTrackedFunction(Function *F) {
  Visitor->addArgumentTrackedFunction(F);
}

bool SCCPTauSolver::isArgumentTrackedFunction(Function *F) {
  return Visitor->isArgumentTrackedFunction(F);
}

void SCCPTauSolver::solve() { Visitor->solve(); }

bool SCCPTauSolver::resolvedUndefsIn(Function &F) {
  return Visitor->resolvedUndefsIn(F);
}

bool SCCPTauSolver::isBlockExecutable(BasicBlock *BB) const {
  return Visitor->isBlockExecutable(BB);
}

bool SCCPTauSolver::isEdgeFeasible(BasicBlock *From, BasicBlock *To) const {
...
void SCCPTauSolver::removeLatticeValueFor(Value *V) {
...
SmallPtrSetImpl<Function *> &SCCPTauSolver::getArgumentTrackedFunctions() {
  return Visitor->getArgumentTrackedFunctions();
}

void SCCPTauSolver::markArgInFuncSpecialization(Function *F, Argument *A,
                                             Constant *C) {
  Visitor->markArgInFuncSpecialization(F, A, C);
}

void SCCPTauSolver::markFunctionUnreachable(Function *F) {
  Visitor->markFunctionUnreachable(F);
}

/// COMMENT : Visits PHINode as well.
void SCCPTauSolver::visit(Instruction *I) {
  LLVM_DEBUG(dbgs() << "[taulog] visitI() Visiting LLVM Instrinsic : llvm.tau (" 
    << I->getOpcodeName() << ")\n");
  CallInst* CI = dyn_cast<CallInst>(I);
  if (CI != NULL) {
    Function* CF = CI->getCalledFunction();
...
/// COMMENT : Handle processing of Tau Inst for SSCCP Solver class
void SCCPTauSolver::visitCall(CallInst &I) { 
  LLVM_DEBUG(dbgs() << "[taulog] visitCall() Visiting LLVM Instrinsic : llvm.tau (" 
    << I.getOpcodeName() << ")\n");
  CallInst* CI = dyn_cast<CallInst>(&I);
  if (CI != NULL) {
    Function* CF = CI->getCalledFunction();
